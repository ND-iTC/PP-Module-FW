...
[width="100%",cols="16%,84%",]
|===
|[PP-ND] |collaborative Protection Profile for Network Devices, Version 4.0, 25 November 2025
|[SD-FW] |Evaluation Activities for Stateful Traffic Filter Firewalls PP-Configuration, Version 2.0, 30 March 2026
|[SD-ND] |Evaluation Activities for Network Device cPP, Version 4.0, 25 November 2025
|===

*Revision History*

[width="100%",cols="17%,19%,63%",options="header",]
|===
|*Version* |*Date* |*Description* 
|2.0 |30-March-2026 |Version for NDcPP V4.0
|1.4e |25-June-2020 |Added link to allowed-with list for FW Module
...
// Figure references
<<fig-protected-comm,Figure 1: Protected Communications SFR Architecture>>

<<fig-mgmt-arch,Figure 2: Management SFR Architecture>>

<<fig-firewall-arch,Figure 3: Firewall Rules SFR Architecture>>

*List of Tables*

// Table references
<<tbl-dist-toe,Table 1: Additional SFRs for Distributed TOEs>>

<<tbl-audit,Table 2: SFRs and Auditable Events>>

<<tbl-opt-audit,Table 3: TOE Optional SFRs and Auditable Events>>

<<tbl-deps-mandatory,Table 4: SFR Dependencies Rationale for Mandatory SFRs>>

<<tbl-deps-optional,Table 5: SFR Dependencies Rationale for Optional SFRs>>
...
The scope of this PP-Module is to describe the security functionality of a firewall device in terms of [CC] and to define functional and assurance requirements for such products. This PP-Module is intended for use with the following PP-Module Base:

* Collaborative Protection Profile for Network Device (NDcPP) Version 4.0

A firewall is a specialized network device that is deployed on the edge of a trusted internal network and untrusted external network and is designed to monitor, filter, and control incoming and outgoing network traffic based on a defined set of security rules.
...
The PP-Module specifies only one PP-Module Base and fully inherits the conformance claim of the PP-Module Base.

The PP-Module specifies a TOE type which is a specific instantiation ('Stateful Traffic Filter Firewall') of the general TOE type defined in the PP-Module Base ('Network Device'). The PP-Module does not interpret any element of the PP-Module Base except for the impact of the stricter TOE type definition.

The PP-Module specifies only additional threats which are independent from the threats of the PP-Module Base and does not refine or interpret any threat defined in the PP-Module Base. The additional threats are mapped only to additional SFRs specified in this PP-Module (not to any SFRs defined in the PP-Module Base) as well as some supporting SFRs (FAU_GEN.1, FMT_SMF.1/FFW) that extend existing SFRs in the PP-Module Base to cover the needs of the SFRs additionally defined in this PP-Module. So PP-Module Base and PP-Module are fully consistent regarding the threat definition.

The PP-Module specifies no additional assumptions compared to the PP-Module Base and does not refine or interpret any assumption defined in the PP-Module Base. The assumption A.NO_THRU_TRAFFIC_PROTECTION defined in the PP-Module Base applies to the traffic allowed to pass through the firewall only. The PP-Module applies this assumption consistently within the context of the Stateful Traffic Filter Firewall TOE type. The PP-Module Base and PP-Module therefore do not conflict with each other.

The PP-Module does not specify any additional Organizational Security Policies and does not refine or interpret any Organizational Security Policy defined in the PP-Module Base.

The PP-Module Base maps threats and Organizational Security Policies directly to SFRs. The mapping of SPD to implicit TOE security objectives as in the PP-Module Base continues to hold, and this PP-Module only adds security objectives.

The PP-Module does not specify any additional Security Objectives for the Environment and does not refine or interpret any Security Objectives for the Environment defined in the PP-Module Base. So PP-Module Base and PP-Module are consistent in this regard.

This PP-Module specifies only additional SFRs which are independent from the SFRs of the PP-Module Base except for the additional audit events specified for FAU_GEN.1 and the additional security management capabilities specified in FMT_SMF.1/FFW. The SFRs in the PP-Module add additional functionality, but do not overlap with the functionality or objects in the SFRs from the PP-Module Base. The PP-Module does not refine or modify any SFRs defined in the PP-Module Base. Therefore, the PP-Module is consistent with the PP-Module Base with regard to the SFRs.

All in all, this rationale demonstrates that a TOE can meet the TOE type descriptions provided in the PP-Module Base and in the PP-Module. A TOE can satisfy all the PP-Module Bases and the PP-Module SFRs. This consistency rationale demonstrates that the unions of the SPD, the objectives and the SFRs from the PP-Module Base and from the PP-Module do not lead to any contradiction. Due to the clear separation between the requirements of the PP-Module Base and the requirements of this PP-Module any correspondence tables would be trivial and have therefore been omitted.
...
As defined by the references [CC1], [CC2], [CC3], [CC4], [CC5], [CEM] and [CC-E&I], this PP-Module:
...
This PP-Module inherits Exact Conformance as required from the specified PP-Module Base and as defined in [Exact Conformance Addendum]. This means that STs must claim exact conformance to PP-Configurations including this PP-Module and its PP-Module Base. This PP-Module requires the use of evaluation methods/evaluation activities defined in [SD-FW] and [SD-ND]. 

The 'FW Module allowed-with list' that specifies the packages and PP-Modules to which exact conformance can be claimed in conjunction with this Module, can be found at https://nd-itc.github.io/AWL/FW_allowed_with_list.html.
...
The threats for the Stateful Traffic Filter Firewall are grouped according to functional areas of the device in the sections below. The description of each threat is then followed by a rationale describing how it is addressed by the SFRs in Section 6 and Annex A.

The threats defined in the PP-Module Base are not replicated here because they are fully covered by SFRs in the PP-Module Base.
...
An attacker may attempt to “map” a subnet to determine the machines that reside on the network, and obtain the IP addresses of machines, as well as the services (ports) those machines are offering. This information could be used to mount attacks to those machines via the services that are exported.

SFR Rationale:

* Requirements to prevent unauthorized disclosure of network information are defined in FFW_RUL_EXT.1 and FFW_RUL_EXT.2 supported by FMT_SMF.1/FFW.
...
An attacker may attempt to connect to network servers inside a protected network providing services for external consumption from a network that is not intended to access these servers or services or via ports that are not intended to access these servers or services. An attacker may provide potentially malicious services in an external network that are not supposed to be accessed from within a protected network or only be accessed via specific ports.

SFR Rationale:

* Requirements to prevent unauthorized access to protected devices and services are defined in FFW_RUL_EXT.1 and FFW_RUL_EXT.2 supported by FMT_SMF.1/FFW
...
Unauthorized agents may attempt to access network elements (e.g., virtual machines, internal networks, or interfaces) in a way that contravenes a defined security rule. This could result in the attacker finding an avenue (e.g., misconfiguration, flaw, or weak credential) to compromise a network element.
...
All Assumptions of the PP-Module Base apply also to this PP-Module. A.NO_THRU_TRAFFIC_PROTECTION is still operative, but only for the traffic allowed to pass through the firewall. This PP-Module extends the PP-Module Base threats (in Section 4.1) to deal with traffic passing through the firewall, and addresses these additional threats directly with the SFRs (FDP_RIP.2, FFW_RUL_EXT.1, FFW_RUL_EXT.2, FMT_SMF.1/FFW) in Section 6 and Annex A.

=== Organizational Security Policy

An organizational security policy is a set of rules, practices, and procedures imposed by an organization to address its security needs. All organizational security policies of the PP-Module Base apply also to this PP-Module. No additional policies are defined.
...
All objectives for the Operational Environment of the PP-Module Base apply also to this PP-Module. OE.NO_THRU_TRAFFIC_PROTECTION is still operative, but only for the traffic allowed to pass through the firewall (i.e., no additional protection for allowed traffic).
...
|OE.VM_CONFIGURATION |A.VS_CORRECT_CONFIGURATION, A.VS_ISOLATION
|===
...
|A.VS_ISOLATION (applies to vNDs only) |The assumption that the VS provides and is configured to provide sufficient isolation between software running in VMs on the same physical platform is addressed by the corresponding requirement in OE.VM_CONFIGURATION
...
The individual SFRs are specified in the sections below. SFRs in this section are mandatory SFRs that any conformant TOE must meet. Additional optional SFRs may also be adopted from those listed in Annex A.

For chapters 6.3 to 6.5 information is provided in each chapter whether the PP-Module specifies additional SFRs not existent in the PP-Module Base or whether the definitions in the PP-Module impact SFRs already existing in the PP-Module Base. The ST author has full discretion to complete all other assignments/selections, include/exclude optional SFRs/etc. of the PP-Module Base as desired because this PP-Module doesn’t impact them.
...
* Assignment completed within a selection in the PP: the completed assignment text is indicated with [.underline]#_italicized and underlined text_#
+

e.g. [selection: _change_default, query, modify,
delete, [assignment: other operations]_
]’ in [CC2] or an ECD might become ‘[.underline]#change_default#, [.underline]#_select_tag_#’ (completion of both selection and assignment) or ‘[selection: [.underline]#change_default#, [.underline]#_select_tag, select_value_#]’ (partial completion of selection, and completion of assignment) in the PP;
...
An overview of the SFR Architecture can be found in the PP-Module Base [PP-ND, 6.2]. The figure about Protected Communications SFR Architecture in the PP-Module Base needs to be replaced by Figure 1 below because the PP-Module adds protection of packet contents (FDP_RIP.2). In addition to the SFR Architecture described in the PP-Module Base, Figure 3 below needs to be considered since it reflects the Firewall Rules SFR Architecture.

SFRs in Annex A can be included in the ST if they are provided by the TOE but are not mandatory in order for a TOE to claim conformance to this PP-Module.

image:extracted-media/FW/media/protected_comms.png[image,width=552,height=374]

[[fig-protected-comm]]
_Figure 1: Protected Communications SFR Architecture_

image:extracted-media/FW/media/management.png[image]

[[fig-mgmt-arch]]_Figure 2: Management SFR Architecture_

image:extracted-media/FW/media/firewall.png[image]

[[fig-firewall-arch]]_Figure 3: Firewall Rules SFR Architecture_

=== Security Audit (FAU)

For FAU there are no new SFRs defined in this PP-Module but there is an impact on SFRs existing already in the PP-Module Base as follows.

==== Security Audit Data Generation (FAU_GEN)

The SFRs of the FAU class as specified in the PP-Module Base apply also for this PP-Module. The table in the PP-Module Base specifying the auditable events and additional audit record contents for FAU_GEN.1 shall be amended by the following entries. Support of all auditable events defined in the table below is mandatory.
...
[[tbl-audit]]_Table 2: SFRs and Auditable Events_

*_Application Note {counter:appnote_count}_*

_Additional audit events will apply to the TOE depending on the optional requirements adopted from Annex A. The ST author must therefore include the relevant additional events specified in Table 3 when the optional SFR FFW_RUL_EXT.2 is claimed._

=== User Data Protection (FDP)

This section requires the TOE to ensure that it does not reuse old packet information when transmitting new packets. The SFRs in this chapter are additional SFRs defined in this PP-Module which are not present in the PP-Module Base.
...
The SFRs in this chapter are additional SFRs defined in this PP-Module which are not present in the PP-Module Base.
...
. _The TSF shall drop and be capable of logging packets where the source address of the network packet is defined as being on a broadcast network for IPv4;_
...
. _The TSF shall drop and be capable of logging network packets where the source or destination address of the network packet is defined as being unspecified (i.e. 0.0.0.0) or an address “reserved for future use” (i.e. 240.0.0.0/4) as specified in RFC 5735 for IPv4. This requirement does not apply to IPv4 DHCP packets;_
. _The TSF shall drop and be capable of logging network packets where the source or destination address of the network packet is defined as an “unspecified address” or an address “reserved for future definition and use” (i.e. unicast addresses not in this address range: 2000::/3) as specified in RFC 3513 for IPv6. This requirement does not apply to IPv6 Duplicate Address Detection (DAD) related packets;_
...
In this chapter management functions are specified that shall be supported by the TOE in addition to the ones specified in (and selected from) the PP-Module Base.
...
_This management function needs to be supported by the TSF in addition to the ones specified in (and selected from) FMT_SMF.1.1 in the PP-Module Base._

[cols="35%,65%",options="header"]
|===
| FMT_SMF.1.1/FFW Management Function
| Management Function Guidance
| _Ability to configure firewall rules_
| _The ability to configure firewall rules is expected to be restricted to authorized Security Administrators in accordance with the role-based administrative model defined in the PP-Module Base._
|===
...
The Security Assurance Requirements are fully inherited from the PP-Module Base. This PP-Module requires the use of evaluation methods/evaluation activities defined in [SD-FW] and [SD-ND].
...
As indicated in the introduction to this PP-Module, the baseline requirements (those that must be performed by the TOE) are contained in the body of this PP-Module. Additionally, there are optional requirements specified in this Annex.

The optional requirements in this Annex comprise requirements that can be included in the ST but are not mandatory for a TOE to claim conformance to a PP-Configuration that includes this PP-Module.
...
|None
|===
[[tbl-opt-audit]]_Table 3: TOE Optional SFRs and Auditable Events_
...
There are no selection-based requirements defined for this PP-Module. This chapter is only kept for consistency with the structure of the PP-Module Base.
...
This Annex contains the definitions for the extended requirements that are used in the PP-Module, including those used in Annex A.
...
image::extracted-media/FW/media/FFW_RUL_EXT.png[width=80%]
...
|===
[[tbl-deps-mandatory]]_Table 4: SFR Dependencies Rationale for Mandatory SFRs_
...
|===
[[tbl-deps-optional]]_Table 5: SFR Dependencies Rationale for Optional SFRs_
...
|*Dynamic Protocol*
|A protocol that requires additional connections or sessions to be dynamically established based on information exchanged over an existing connection (e.g., FTP, SIP, H.323). A Stateful Traffic Filter Firewall supporting such protocols must dynamically permit the additional flows required by the protocol.

|*Filtering*
|The process by which the TOE evaluates network traffic against a defined ruleset to determine whether the traffic is permitted to pass or is dropped.

|*Ruleset*
|An ordered collection of rules that defines the traffic filtering policy enforced by the TOE. Each rule specifies matching criteria and the action to be applied to traffic that matches the rule (e.g., permit or drop).
