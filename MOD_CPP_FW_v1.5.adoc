...
|[SD-FW] |Evaluation Activities for Stateful Traffic Filter Firewalls PP-Configuration, Version 2.0, TBD March 2026
...
|2.0 |TBD-March-2026 |Version for NDcPP V4.0
...
A firewall is a specialized network device that deployed on the edge of a trusted internal network and untrusted external network and is designed to monitor, filter, and control incoming and outgoing network traffic based on a defined set of security rules.
...
The PP-Module specifies a TOE type which is a specific instantiation ('Stateful Traffic Filter Firewall') of the more general TOE type defined in the PP-Module Base ('Network Device'). The PP-Module does not interpret any element of the PP-Module Base except for the impact of the more stricter TOE type definition.
...
|A.ADMIN_CREDENTIALS_SECURE |TThe assumption that the Security Administrator’s credentials are protected by the platform they are stored on is addressed by the corresponding requirement in OE.ADMIN_CREDENTIALS_SECURE.
