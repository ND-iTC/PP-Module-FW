...
|[SD-FW] |Evaluation Activities for Stateful Traffic Filter Firewalls PP-Configuration, Version 2.0 draft, 02 December 2025
...
|2.0 draft |02-December-2025 |Version for NDcPP V4.0
...
The PP-Module Base does not define explicit Objectives for the TOE, but maps threats and Organisational Security Policies directly to SFRs. In this PP-Module, explicit security objectives for the TOE are defined and mapped to the additional threats for the PP-Module (the TOE security objectives are then mapped to the additional SFRs in this PP-Module). The mapping of SPD to implicit TOE security objectives as in the PP-Module Base continues to hold, and this PP-Module only adds security objectives. Therefore, the security objectives for the TOE defined in this PP-Module do not contradict the security objectives for the TOE in the PP-Module Base.
...
With knowledge of the services that are advertised by machines on a subnet, an attacker may attempt to exploit those services by mounting attacks against the machines offering them.
...
An attacker may attempt to use services that are advertised by machines in a way that is unintended by a site’s security policies. For example, an attacker might be able to use a service to “anonymize” the attacker’s machine as they mount attacks against others.
...
All Assumptions of the PP-Module Base apply also to this PP-Module. A.NO_THRU_TRAFFIC_PROTECTION is still operative, but only for the interfaces in the TOE that are defined by the PP-Module Base and not the PP-Module. This PP-Module extends the PP-Module Base threats (in section 4.1) to deal with traffic passing through the firewall, and addresses these additional threats with the TOE Security Objectives in section 5.1 and the SFRs (FDP_RIP.2, FFW_RUL_EXT.1, FFW_RUL_EXT.2, FMT_SMF.1/FFW) in section 6 and Annex A.
...
The following subsections describe objectives for the TOE. Since the PP-Module Base does not specify any Objectives for the TOE this section contains only additional Objectives for the TOE related to the PP-Module but independent from the PP-Module Base.
...
All objectives for the Operational Environment of the PP-Module Base apply also to this PP-Module. OE.NO_THRU_TRAFFIC_PROTECTION is still operative, but only for the interfaces in the TOE that are defined by the PP-Module Base and not the PP-Module.
...
|===
[[tbl-coverage-mandatory]]_Table 6: SFR Coverage Mapping for Mandatory SFRs_
...
|===
[[tbl-coverage-optional]]_Table 7: SFR Coverage Mapping for Optional SFRs_
...
|===
[[tbl-sufficiency]]_Table 8: SFR Sufficiency Rationale_
