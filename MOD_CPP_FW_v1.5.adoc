...
|[SD-FW] |Evaluation Activities for Stateful Traffic Filter Firewalls PP-Configuration, Version 2.0, 02 December 2025
...
|2.0 |02-December-2025 |Version for NDcPP V4.0
...
PP-Module Date: 02-December-2025
...
All Assumptions of the PP-Module Base apply also to this PP-Module. A.NO_THRU_TRAFFIC_PROTECTION is still operative, but only for the interfaces in the TOE that are defined by the PP-Module Base and not the PP-Module. This PP-Module extends the PP-Module Base threats (in Section 4.1) to deal with traffic passing through the firewall, and addresses these additional threats directly with the SFRs (FDP_RIP.2, FFW_RUL_EXT.1, FFW_RUL_EXT.2, FMT_SMF.1/FFW) in Section 6 and Annex A.
