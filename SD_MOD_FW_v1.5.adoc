...
[arabic, start=32]
. If the TOE does not support the configuration of all possible values in the specified network protocol fields, the evaluator shall verify the TSS: identifies which fields cannot filter all values; and specifies, for those fields, which values cannot be filtered (implicitly or explicitly) by rules. For any field and value combinations that are not supported, evaluator shall confirm that any values described as unsupported or unfilterable are clearly identified in the TSS and do not undermine the TOE’s ability to enforce its security policy. If the TOE supports the implicit configuration of any values, the evaluator shall ensure the TSS identifies the specific configurations that result in the implicit configuration of a value (e.g., configuring ICMP type 8 implicitly configures ICMP code 0). 
...
[arabic, start=79]
. The evaluator shall verify that the TSS identifies the protocols that can cause the automatic creation of dynamic packet filtering rules. In some cases rather than creating dynamic rules, the TOE might establish stateful sessions to support some identified protocol behaviours.
